...
// Custom icon components to replace RemixIcon dependency
const DownloadIcon = () => (
...
export default function SharePortfolio() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // In a real application, you would integrate with a PDF library like jsPDF or html2pdf
  ...
  const handleSocialShare = (platform) => {
    ...
    const shareUrls = {
      ...
      email: `mailto:?subject=${encodeURIComponent('Portfolio Share')}&body=${encodeURIComponent(`${message}\n\n${url}`)}`
    };
      ...
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        <div className="mb-12">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto leading-relaxed">
            Export your portfolio as a PDF or share it directly with potential
            clients and collaborators to showcase your professional work.
          </p>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-8 shadow-lg max-w-md mx-auto backdrop-blur-sm bg-opacity-95">
            ...
            <div className="grid grid-cols-4 gap-4 mb-8">
              {socialPlatforms.map((platform) => (
                ...
                >
                  <div className={`w-14 h-14 rounded-full ${platform.bgColor} flex items-center justify-center ${platform.textColor} mb-3 shadow-md group-hover:shadow-lg transition-shadow duration-200`}>
                    {platform.icon}
                  </div>
            ...
            <div>
              ...
              <div className="flex rounded-lg overflow-hidden border border-gray-300 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500">
                <input
                  type="text"
                  value={portfolioUrl}
                  className="flex-1 px-4 py-3 text-sm focus:outline-none bg-gray-50"
                  readOnly
                />
                <button
                  onClick={handleCopyLink}
                  className={`px-4 py-3 transition-all duration-200 font-medium ${
                    copySuccess 
                      ? 'bg-green-600 hover:bg-green-700' 
                      : 'bg-blue-600 hover:bg-blue-700'
                  } text-white flex items-center justify-center min-w-[60px]`}
                >
