...
const pressureLevels = {
  1: {
    ...
    description: "Project timeline will be extended to 120% of the estimated time. Reminders will be minimal.",
    color: "emerald",
    icon: RiPlantLine,
  ...
  2: {
    ...
    description: "Project timeline will be slightly extended. You'll get friendly nudges.",
    color: "green",
    icon: RiLeafLine,
  ...
  3: {
    ...
    description: "Project timeline will be adjusted to 80% of the estimated completion time. You'll receive occasional reminders.",
    color: "blue",
    icon: RiScalesLine,
  ...
  5: {
    ...
    description: "Project deadline will be tight. Expect urgent reminders and tight feedback loops.",
    color: "red",
...
const DeadlinePressureSlider = ({ value, onChange }) => {
      ...
      dots.push(
        <div
          ...
          className={`w-3 h-3 rounded-full transition-all duration-300 ${
            i <= level
              ? `bg-${current.color}-500 ring-2 ring-${current.color}-200`
              : 'bg-gray-200'
          }`}
        />
      );
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-8 hover:shadow-md transition-shadow duration-300">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 bg-gradient-to-br ${getSliderTrackColor(value)} rounded-xl flex items-center justify-center shadow-sm`}>
        ...
        <div className={`px-4 py-2 bg-${current.color}-50 border border-${current.color}-200 rounded-full flex items-center gap-2`}>
          <current.icon className={`text-lg text-${current.color}-600`} />
      ...
      <p className="text-gray-600 mb-8 leading-relaxed">
        Adjust how much time pressure you'll experience during the simulation. This affects timeline expectations and reminder frequency.
      </p>
      ...
      <div className="mb-8">
        <div className="relative">
          ...
          <div className="h-3 bg-gray-100 rounded-full relative overflow-hidden">
            <div 
              className={`h-full bg-gradient-to-r ${getSliderTrackColor(value)} rounded-full transition-all duration-500 ease-out`}
      ...
      <div className="flex justify-center gap-2 mb-8">
        {getPressureIndicator(value)}
      </div>

      {/* Current Setting Info Card */}
      <div className={`bg-gradient-to-r from-${current.color}-50 to-${current.color}-50/50 border border-${current.color}-200 rounded-xl p-6 relative overflow-hidden`}>
        {/* Background pattern */}
        ...
        <div className="relative">
          ...
          <div className="flex items-start gap-4 mb-4">
            <div className={`w-10 h-10 bg-${current.color}-100 rounded-xl flex items-center justify-center flex-shrink-0`}>
              <RiInformationLine className={`text-lg text-${current.color}-600`} />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <current.icon className={`text-xl text-${current.color}-600`} />
                <h4 className={`text-lg font-semibold text-${current.color}-900`}>
                  {current.label}
                </h4>
              </div>
              <p className={`text-${current.color}-700 leading-relaxed`}>
                {current.description}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <div className={`flex items-center gap-3 p-3 bg-white/60 rounded-lg border border-${current.color}-200/50`}>
              <RiTimeLine className={`text-${current.color}-600`} />
              <div>
                <div className="text-xs font-medium text-gray-600 uppercase tracking-wide">Timeline</div>
            ...
            <div className={`flex items-center gap-3 p-3 bg-white/60 rounded-lg border border-${current.color}-200/50`}>
              ...
              <div>
                <div className="text-xs font-medium text-gray-600 uppercase tracking-wide">Reminders</div>
...
export default DeadlinePressureSlider;